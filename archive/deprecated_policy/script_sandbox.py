...
This module provided subprocess-based script execution with best-effort
detection of file access. It has been superseded by the monkey-patch
sandbox in ck3lens.tools.script_sandbox that can actually PREVENT
out-of-scope I/O rather than just detecting it after the fact.

For ck3lens mode, use the tools-layer sandbox that:
- Intercepts I/O at runtime via monkey-patching
- Delegates to WorldAdapter.is_visible() for reads
- Delegates to enforcement.gate_write() for writes
- Prevents rather than just detects policy violations

Migration:
    # Old (subprocess, detection-only)
    from ck3lens.policy.script_sandbox import execute_wip_script
    
    # New (monkey-patch, prevention)
...
warnings.warn(
    "ck3lens.policy.script_sandbox is deprecated. "
    "Use ck3lens.tools.script_sandbox instead.",
    DeprecationWarning,
    stacklevel=2,
)

# Original code below kept for reference but should not be used
# =========================================================================

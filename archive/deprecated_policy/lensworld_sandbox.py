"""
DEPRECATED - Use ck3lens.tools.script_sandbox instead

This module has been superseded by the tools-layer sandbox that properly
delegates to WorldAdapter.is_visible() for reads and enforcement.gate_write()
for writes.
...
The new sandbox:
- Lives in tool layer (not policy layer)
- Uses WorldAdapter.is_visible() for read visibility
- Uses enforcement.py for write permission
- Has simpler signature: session instead of individual paths
"""
...
from ..tools.script_sandbox import (
...
warnings.warn(
    "ck3lens.policy.lensworld_sandbox is deprecated. "
    "Use ck3lens.tools.script_sandbox instead.",
    DeprecationWarning,
    stacklevel=2,
)

# Keep old imports for any remaining references
import builtins
import os
import sys
...
class LensWorldSandbox:
    ...
    def __init__(
        ...
        Args:
            ...
            local_mods_folder: The folder containing local mods (read allowed, write if declared)
        ...
        self.local_mods_folder = local_mods_folder.resolve() if local_mods_folder else None
        ...
        read_roots = {self.wip_path} | self.utility_paths
        if self.local_mods_folder:
            read_roots.add(self.local_mods_folder)
        self.allowed_read_roots = read_roots

        # Build combined write paths (WIP always, plus declared paths if under local_mods_folder)
        self.allowed_write_roots = {self.wip_path}
        for write_path in self.declared_write_paths:
            # Check if write_path is under local_mods_folder
            if self.local_mods_folder:
                try:
                    write_path.relative_to(self.local_mods_folder)
...
def run_script_sandboxed(
    ...
    Args:
        ...
        local_mods_folder: Folder containing local mods (read allowed, write if declared)

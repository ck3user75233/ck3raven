"""Write updated linter_lock.py with proposed lock and diff functions."""
...
if __name__ == "__main__":
    import sys
    # Write to ck3raven repo
    repo_root = Path(__file__).parent.parent.parent  # ~/.ck3raven/wip -> ck3raven
    if not (repo_root / "pyproject.toml").exists():
        # Try to find it
        for p in Path(__file__).resolve().parents:
            if (p / "ck3raven" / "pyproject.toml").exists():
                repo_root = p / "ck3raven"
                break
    
    # Fallback to explicit path
    target = Path(r"C:\Users\nateb\Documents\CK3 Mod Project 1.18\ck3raven\tools\compliance\linter_lock.py")

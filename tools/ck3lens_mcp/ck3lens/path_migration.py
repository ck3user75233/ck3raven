...
def validate_paths_exist(playset_data: dict) -> list[dict]:
    """
    Validate that paths in playset actually exist on disk.
    
    Returns list of issues: [{path, key, exists, message}]
    """
    issues = []
    
    # Check local_mods_folder
    local_mods = playset_data.get("local_mods_folder", "")
    if local_mods:
        exists = Path(local_mods).exists()
        if not exists:
            issues.append({
                "key": "local_mods_folder",
                "path": local_mods,
                "exists": False,
    ...
    # Check vanilla path
    vanilla = playset_data.get("vanilla", {})
    vanilla_path = vanilla.get("path", "")
    if vanilla_path:
        exists = Path(vanilla_path).exists()
        if not exists:
            issues.append({
                "key": "vanilla.path",
                "path": vanilla_path,
                "exists": False,
                "message": f"Vanilla game path does not exist: {vanilla_path}"
            })
    
    # Check mod paths (only check a sample to avoid slowdown)
    mods = playset_data.get("mods", [])
    for i, mod in enumerate(mods[:5]):  # Check first 5
        mod_path = mod.get("path", "")
        if mod_path:
            exists = Path(mod_path).exists()
            if not exists:
                issues.append({
                    "key": f"mods[{i}].path",
                    "path": mod_path,
                    "exists": False,
                    "message": f"Mod path does not exist: {mod.get('name', 'unknown')} at {mod_path}"
                })
